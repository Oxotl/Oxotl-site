...
(function () {
  ...
  function selectSession(sessionId, expand) {
    ...
    var demo = document.querySelector("[data-island-demo]");
    if (demo) demo.classList.toggle("is-buy-session", selectedSessionId === "buy");
  ...
  if (document.body.dataset.page === "download") {
    var target = config.downloadUrl;
    var anchor = document.querySelector("[data-download-target]");
    var directDownload = document.querySelector("[data-direct-download-link]");
    var directDownloadLabel = document.querySelector("[data-direct-download-label]");
    var messageMain = document.querySelector("[data-download-message-main]");
    var messageFallback = document.querySelector("[data-download-message-fallback]");
    var isMac = isMacBrowser();
    if (anchor && target && !isPlaceholder(target)) {
      anchor.textContent = target;
    }

    if (!isMac) {
      if (messageMain) messageMain.textContent = "Open this page on macOS to download Oxotl.";
      if (messageFallback) messageFallback.textContent = "Downloads are only available from a Mac.";
      if (directDownload) {
        if (directDownloadLabel) directDownloadLabel.textContent = "Download on macOS";
